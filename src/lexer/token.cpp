...
namespace soul
{
	...
	Token::Token(TokenType type, value_t value) : _type(type), _value(std::move(value)) {}
	...
	Token::operator std::string() const
	{
		const auto& value_string = std::visit(
			...
			_value);
		return std::string(std::format("[type: {}, value: {}]", token_type_to_string_view(_type), value_string));
	}
}  // namespace soul
